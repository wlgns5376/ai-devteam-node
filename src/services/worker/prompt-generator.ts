...
## 응답 형식
작업이 완료되면 **반드시** 다음 형식으로 응답해주세요:

\`\`\`
---TASK_RESULT_START---
상태: [성공 또는 실패]
메시지: [작업이 성공적으로 완료되었습니다 / 작업 처리 중 오류가 발생했습니다]

## 작업 진행 상황 요약
[구현한 기능과 변경 사항을 상세히 기술]

## PR 링크
PR: https://github.com/...

## 주요 변경 사항
[추가/수정/삭제된 파일과 핵심 로직 설명]
...
**중요**: 위 형식을 정확히 따라주세요. 특히 \`---TASK_RESULT_START---\`와 \`---TASK_RESULT_END---\` 구분자는 필수입니다.
...
## 응답 형식
모든 피드백 처리가 완료되면 **반드시** 다음 형식으로 응답해주세요:

\`\`\`
---TASK_RESULT_START---
상태: [성공 또는 실패]
메시지: [피드백이 성공적으로 반영되었습니다 / 피드백 처리 중 오류가 발생했습니다]
...
## 응답 형식
병합이 완료되면 **반드시** 다음 형식으로 응답해주세요:

\`\`\`
---TASK_RESULT_START---
상태: [성공 또는 실패]
메시지: [병합이 성공적으로 완료되었습니다 / 병합 처리 중 오류가 발생했습니다]

## 병합 결과
[성공 여부와 병합 방식]

## 발생한 충돌과 해결 방법
[충돌이 있었다면 해결 과정, 없으면 "충돌 없이 병합 완료"]
